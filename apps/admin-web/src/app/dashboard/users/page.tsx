    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                ...
                // 買い物代行者を除外（一般ユーザーのみを表示）
                const regularUsers = usersData.filter((user: any) => 
                    !user.email.includes('shopper') && !user.email.includes('買い物代行者')
                );
                
                // ユーザーデータを整形
                const formattedUsers = regularUsers.map((user: any) => ({
                    ...user,
                    name: user.email.split('@')[0], // メールアドレスから名前を生成
                    role: 'customer', // 一般ユーザーは顧客として分類
            ...
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-gray-600">一般ユーザー（顧客）を管理できます</p>
                    </div>
                </div>
